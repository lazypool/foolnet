...
/* harmony export */   "default": () => (/* binding */ applescript_tmLanguage),
...
const repository = {
	...
	finder: {
		patterns: [
			...
			{
				match: "\\b((Finder|desktop|information|preferences|clipping) )({})s?\\b",
				name: "support.class.finder.({})-classes.applescript"
			},
			{
	...
	itunes: {
		patterns: [
			{
				match: "\\b(artwork|application|encoder|EQ preset|item|source|visual|(EQ |browser )?({})|((audio CD|device|shared|URL|file) )?track|playlist ({})|((audio CD|device|radio tuner|library|folder|user) )?playlist)s?\\b",
	...
	"standard-suite": {
		patterns: [
			{
				match: "\\b(colors?|documents?|items?|({})s?)\\b",
	...
	standardadditions: {
		patterns: [
			...
			{
				match: "\\b(opening folder|(closing|moving) folder ({}) for|adding folder items to|removing folder items from)\\b",
	...
	"system-events": {
		patterns: [
			...
			{
				match: "\\b(((application |desk accessory )?process|(check|combo )?box)(es)?|(action|attribute|browser|(busy|progress|relevance) indicator|color well|column|drawer|group|grow area|image|incrementor|list|menu( bar)?( item)?|(menu |pop up |radio )?button|outline|(radio|tab|splitter) group|row|scroll (area|bar)|sheet|slider|splitter|static text|table|text (area|field)|tool bar|UI element|({}))s?)\\b",
